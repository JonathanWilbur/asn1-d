// D import file generated from '.\source\asn1\codecs\ber.d'
...
public class BasicEncodingRulesElement : ASN1Element!BERElement, Byteable
{
	...
	public override const @property bool[] bitString();
	public override @property void bitString(in bool[] value);
	public override const @property @system ubyte[] octetString();
	public override @property @safe void octetString(in ubyte[] value);
			...
			{
				{
					...
					numericRepresentation = numericRepresentation.stripLeft().replace(",", ".");
						...
						{
							{
								if (this.value.length < 5u)
									throw new ASN1TruncationException(5u, this.value.length, "decode a REAL exponent");
								...
								{
									throw new ASN1ValueOverflowException("This exception was thrown because you attempted " ~ "to decode a REAL that had an exponent that was " ~ "too big to decode to a floating-point type." ~ "Specifically, the exponent was encoded on " ~ "more than two bytes, which is simply too " ~ "many to fit in any IEEE 754 Floating Point " ~ "type supported by this system. " ~ notWhatYouMeantText ~ forMoreInformationText ~ debugInformationText ~ reportBugsText);
								}
								break;
							}
						}
						...
						{
							{
								...
								if (exponentLength == 0u)
									throw new ASN1ValueException("This exception was thrown because you attempted " ~ "to decode a REAL whose exponent was encoded on " ~ "zero bytes, which is prohibited by the X.690 " ~ "specification, section 8.5.6.4, note D. ");
								if (exponentLength > this.length + 2u)
									throw new ASN1TruncationException(exponentLength, this.length + 2u, "decode a REAL from too few bytes to decode its exponent");
								...
								if (exponentLength == 1u)
								{
									...
									if (exponent == 0u)
										throw new ASN1ValueException("This exception was thrown because you " ~ "attempted to decode a REAL whose exponent " ~ "was encoded using the complicated form " ~ "described in specification X.690, section " ~ "8.5.6.4.d, but whose exponent was zero, " ~ "which is prohibited when using the " ~ "complicated exponent encoding form, as " ~ "described in section 8.5.6.4.d. ");
								}
								else
								{
									...
									if (exponentBytes[0] == 0u && !(exponentBytes[1] & 128u) || exponentBytes[0] == 255u && exponentBytes[1] & 128u)
										throw new ASN1ValuePaddingException("This exception was thrown because you attempted to decode " ~ "a REAL exponent that was encoded on more than the minimum " ~ "necessary bytes. " ~ notWhatYouMeantText ~ forMoreInformationText ~ debugInformationText ~ reportBugsText);
									version (LittleEndian)
									{
										exponentBytes = [exponentBytes[1], exponentBytes[0]];
									}
	...
	public const @property @system T enumerated(T)() if (isIntegral!T && isSigned!T)
	{
		...
		if (this.value[0] == 0u && !(this.value[1] & 128u) || this.value[0] == 255u && this.value[1] & 128u)
			throw new ASN1ValuePaddingException("This exception was thrown because you attempted to decode " ~ "an ENUMERATED that was encoded on more than the minimum " ~ "necessary bytes. " ~ notWhatYouMeantText ~ forMoreInformationText ~ debugInformationText ~ reportBugsText);
		immutable ubyte paddingByte = this.value[0] & 128u ? 255u : 0u;
		ubyte[] value = this.value.dup;
		while (value.length < T.sizeof)
		value = paddingByte ~ value;
	...
	public override nothrow @property @system void unicodeTransformationFormat8String(in string value);
	...
	public override const @property @system BERElement[] sequence();
	public override @property @system void sequence(in BERElement[] value);
	public override const @property @system BERElement[] set();
	public override @property @system void set(in BERElement[] value);
	...
	public override nothrow @property @safe void teletexString(in ubyte[] value);
	public override const @property @system ubyte[] videotexString();
	public override nothrow @property @safe void videotexString(in ubyte[] value);
	...
	public override nothrow @property @system void coordinatedUniversalTime(in DateTime value);
	public override const @property @system DateTime generalizedTime();
	public override nothrow @property @system void generalizedTime(in DateTime value);
